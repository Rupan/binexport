...
Operands DecodeOperandsArm(const insn_t& instruction) {
  bool co_processor = instruction.auxpref & aux_coproc ? true : false;
    ...
    switch (operand.type) {
      ...
      case o_idpspec5 + 1: {
        ...
        LOG(INFO) << StringPrintf(
            "warning: text storage not yet supported (%08" PRIx64 ")",
            instruction.ea);
        break;
      }
      default: {
...
Instruction ParseInstructionIdaArm(const insn_t& instruction,
  ...
  if (instruction.itype == ARM_it) {
    unsigned int first_cond = (instruction.ARM_cond & 0x1);
